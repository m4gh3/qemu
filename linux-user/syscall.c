...
/* We might have problem: the AT_EMPTY_PATH flag might screw us over sigh... */
/* qemu_execve() Must return target values and target errnos. */
int qemu_execveat(int dirfd, char *filename, char *argv[],
                  char *envp[], int flags );
int qemu_execveat(int dirfd, char *filename, char *argv[],
                  char *envp[], int flags )
{
	...
	return get_errno(safe_execveat(dirfd, new_argp[0], new_argp, envp, flags ));
    ...
    if (i_name) {
    	...
    	return get_errno(safe_execveat(dirfd, QEMU_TARGET_PATH, new_argp, envp, flags ));
}
                       ...
                       abi_long guest_envp, int flags)
{
    ...
    *q = NULL;
    ...
    if (is_proc_myself(p, "exe")) {
        ret = get_errno(qemu_execveat(dirfd, exec_path, argp, envp, flags));
    } else {
        ret = get_errno(qemu_execveat(dirfd, p, argp, envp, flags));
    }
    #else
    if (is_proc_myself(p, "exe")) {
